...
##counting the no of distinct alphabets in the input string
while [ $current_index -lt $maxindex ]
do
   next=`expr $current_index + 1`;
   if [ "${alphabets:current_index:1}" != "${alphabets:next:1}" ]
   then 
...
##checking if the string has more than 1 distinct alphabets
...
do
    ...
    current_alpha=${alphabets:index:1};
    temp_alpha=${alphabets:temp:1}
    
    ##checking if the same index has been generated again
    ...
    then
        ...
        do
            current_string="$current_string${alphabets:index:1}";
            i=`expr $i + 1`;
        done
